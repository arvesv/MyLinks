...
export const LinkCard: React.FC<LinkCardProps> = ({
  ...
  return (
    <div
      ...
      className="group relative bg-slate-900/70 hover:bg-slate-800/80 border border-slate-800 hover:border-slate-700 rounded-2xl p-4 transition-all duration-200 hover:shadow-xl hover:shadow-indigo-500/5 hover:-translate-y-0.5 flex flex-col justify-between"
    >
      <div>
        {/* Top Header: Icon + Admin Actions */}
        <div className="flex items-start justify-between gap-3 mb-3">
          <div className="w-11 h-11 rounded-xl bg-slate-800/90 border border-slate-700/60 flex items-center justify-center p-2 text-indigo-400 group-hover:scale-105 group-hover:bg-slate-800 transition-all flex-shrink-0">
            ...
            {isAdmin && (
              <>
                ...
                <button
                  ...
                  className="p-1.5 rounded-lg bg-slate-800/80 border border-slate-700 text-slate-400 hover:text-indigo-400 hover:bg-slate-700 transition-colors"
                  title="Edit bookmark"
                >
                ...
                <button
                  ...
                  className="p-1.5 rounded-lg bg-slate-800/80 border border-slate-700 text-slate-400 hover:text-rose-400 hover:bg-slate-700 transition-colors"
        ...
        <a
          ...
          className="block"
        >
          <div className="flex items-center gap-1.5 text-slate-100 font-semibold group-hover:text-indigo-300 transition-colors text-base line-clamp-1">
        ...
        <div className="flex items-center justify-between gap-2 mt-4 pt-3 border-t border-slate-800/60 text-[11px] text-slate-500">
          <div className="flex items-center gap-1 overflow-hidden">
            {tagsList.slice(0, 3).map(tag => (
              <span key={tag} className="px-1.5 py-0.5 rounded-md bg-slate-800 text-slate-400 font-medium">
                #{tag}
              </span>
            ))}
            {tagsList.length > 3 && (
              <span className="text-slate-500">+{tagsList.length - 3}</span>
            )}
          </div>
