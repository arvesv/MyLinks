          ...
          <div>
            <h1 className="text-xl font-bold tracking-tight text-slate-900 dark:text-white flex items-center gap-2">
              ...
              <span
                ...
                title={isOnline ? 'Online — Connected to server' : 'Offline — Showing cached snapshot of your dashboard'}
              >
                {isOnline ? (
                  <>
          ...
          {isAdmin && (
            <div className="flex items-center gap-1.5 ml-1 pl-2 border-l border-slate-200 dark:border-slate-800">
              <button
                ...
                disabled={!isOnline}
                className={`hidden sm:inline-flex items-center gap-1.5 px-2.5 py-1.5 text-xs font-medium border rounded-lg transition-colors ${
                  !isOnline
                    ? 'opacity-40 cursor-not-allowed bg-slate-100 dark:bg-slate-800 text-slate-400 dark:text-slate-500 border-slate-200 dark:border-slate-800'
                    : 'text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white bg-slate-100 dark:bg-slate-800/80 hover:bg-slate-200 dark:hover:bg-slate-800 border-slate-200 dark:border-slate-700/80 cursor-pointer'
                }`}
                title={!isOnline ? 'Cannot manage categories while offline' : 'Manage Categories'}
              >
              ...
              <button
                ...
                title={!isOnline ? 'Cannot export/import backups while offline' : 'Backup & Restore'}
              >
              ...
              <button
                ...
                className={`inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-lg shadow-sm transition-all ${
                  !isOnline
                    ? 'opacity-40 cursor-not-allowed bg-slate-400 dark:bg-slate-700 text-white'
                    : 'text-white bg-indigo-600 hover:bg-indigo-500 cursor-pointer'
                }`}
                title={!isOnline ? 'Cannot add links while offline' : 'Add Link'}
              >
